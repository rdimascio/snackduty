...
| Native visible behavior        | `bun run ios:test:ui`                       | Launch, roster privacy, navigation, event creation, RSVP/claim, empty/error/retry and screenshots using Debug-only fixtures   |
| Native models and UI           | `bun run ios:test`                          | Shared-scheme Swift and UI tests, with guards against zero-test success                                                       |
| Real client/server integration | `bun run accept`                            | Disposable live HTTP journey, native controller/API round trip and real coach–parent UI journey                               |
| Before merging                 | `bun run gate`                              | All required checks, builds, native UI tests, and live acceptance on macOS                                                    |

The fast CI job reports separately from the full macOS job. Both must pass before merging. Fast checks are a subset of the full gate. Fixture UI tests provide fast deterministic screen coverage. Acceptance separately requires named passes for both `liveDevServerRoundTrip` and `LiveCoordinationUITests.testRealCoachParentCoordination`; skipping either is a failure. The live UI journey creates an event as coach, relaunches as a parent, records an own-child RSVP, claims snacks, checks durable server results, and switches to a second team where that adult coaches.

The live UI runner receives a seeded team ID and the loopback origin from acceptance. Its explicit `SNACKDAY_DEV_PERSONA` launch setting is compiled only in Debug and selects one of the server's bounded synthetic adults. These sessions exercise real local APIs but do not verify Apple authentication. For cryptographic identity and invitation integration, `bun test apps/web/test/beta-journey.test.ts` uses generated RS256 tokens with the real verifier, SQL sessions and application operations. Live provider and staging verification remain separate release requirements.
...
Application session restoration must outlive the Schedule view that requested it: switching to a launch/session state removes that view and cancels its task. App-owned work and controller generation checks preserve both restoration and newer selections. Event creation retains the same request ID when retrying an unchanged draft; changing the draft gets a new ID. The server commits the event, optional snack slot and replay receipt together. Use Schedule's explicit refresh control to reload an empty season.

Check the actual simulator screenshots as well as assertions. The first real coordination run exposed fractional-second timestamps displaying “Date unavailable” and a menu row whose empty center did not open its actions. The formatter now covers canonical and whole-second instants; RSVP uses a full-row button and explicit response dialog. `ContentUnavailableView` can propagate its container identifier onto action buttons, so its retry tests select the visible action labels.

Simulator scripts use local ad-hoc signing (`CODE_SIGN_IDENTITY=-`, empty development team), and DomainTests are hosted in the app. Disabling signing or running the real Keychain transport in a hostless test process produced `errSecMissingEntitlement` (-34018) before the first HTTP request. The hosted Keychain test and named live acceptance now exercise actual persistent storage; an in-memory test store cannot replace that evidence. This simulator signature needs no distribution account and is not an iOS device archive or TestFlight signature. See [Apple's unit-test hosting guidance](https://developer.apple.com/forums/thread/773772).
