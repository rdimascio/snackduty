...
type InviteData =
  ...
  | { readonly state: "accepted"; readonly teamName: string; readonly invitedRole: string };

/**
 * The server loader IS the whole preview path — no new public API route
 * exists: the raw token stays in the URL the invited parent already holds, is
 * hashed in-process by `previewInvitation`, and never enters a log or another
 * endpoint's access log. Resolution order:
 *
 ...
 *   - a token THIS signed-in adult already accepted → the success state, so a
 *     refresh or a re-tap of the link stays calm (accept's idempotent-same-
 *     adult semantics, mirrored);
 *   - everything else — unknown, revoked, expired, accepted by someone else —
 *     ONE generic invalid state; the difference is never disclosed.
 */
...
function roleDescription(invitedRole: string): string {
  return invitedRole === "owner" ? "an owner" : "an adult member";
}
...
function Accepted({ invitedRole, teamName }: { invitedRole: string; teamName: string }): ReactNode {
  return (
    <div className="text-center">
      ...
      <p className="mt-2 text-muted-foreground">You joined as {roleDescription(invitedRole)}.</p>
      <div className="mt-8">
        <a
...
function InvitePage(data: InviteData): ReactNode {
  ...
  if (data.state === "accepted") {
    return (
      <InviteShell>
        <Accepted invitedRole={data.invitedRole} teamName={data.teamName} />
      </InviteShell>
    );
  }
